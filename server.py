...
class BackfillInput(BaseModel):
    vocalia: int = Field(..., ge=1, le=8,
                         description="Vocalía a procesar (1-8). El backfill camina el correlativo "
                         "de esa vocalía hacia atrás, reanudando desde donde quedó la vez anterior.")
    max_nuevas: int = Field(15, ge=1, le=40,
                            description="Sentencias nuevas a incorporar en esta llamada. Mantener bajo "
                            "para no exceder el timeout; se llama repetidamente hasta 'agotada'.")


@mcp.tool(description="Backfill histórico reanudable de UNA vocalía, en tramos cortos. Cada llamada "
          "baja hasta 'max_nuevas' sentencias viejas y guarda el progreso; se la vuelve a llamar con "
          "la misma vocalía hasta que el estado sea 'agotada'. Recorrer las vocalías 1 a 8 completa el "
          "histórico. Escribe directo en el índice (volumen). Devuelve estado, nuevas y desglose por año.")
def tfaba_backfill_tramo(input: BackfillInput) -> dict:
    return ingest.backfill_tramo(input.vocalia, max_nuevas=input.max_nuevas)
